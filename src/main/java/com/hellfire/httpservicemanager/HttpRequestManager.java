...
import com.hellfire.customExceptions.CustomException;
import com.hellfire.httpoperations.HttpsCertTrust;
import com.hellfire.httpoperations.WaitCondition;
import com.hellfire.utils.TimeUtil;
...
public class HttpRequestManager {
    ...
    public void onCreate() {
        ...
        if (relaxedHttps) {
            specification.relaxedHTTPSValidation().log().all(true);
    ...
    public  void initNewSpecification() {
        try{
            specification = RestAssured.given().config(restConfig.getConfig()).log().all(true);
            waitCondition = null;
            onCreate();
        }catch(Exception e){
    ...
    public void param(String key, Object value) {specification.param(key, value);}
    ...
    public void queryParam(String parameterName, String parameterValue) {
        ...
        if (!parameterValue.equalsIgnoreCase("null")) {
            specification.queryParam(parameterName, parameterValue).log().all(true);
        }
    }

    public void accept(String mediaTypes) {specification.accept(mediaTypes);}
    ...
    public void contentTypes(String contentType) {specification.contentType(contentType);}

    public void cookie(String cookieName, String cookieValue) {specification.cookie(cookieName,cookieValue);}
    ...
    public void sessionId(String sessionValue) {specification.sessionId(sessionValue);}

    public void sessionId(String sessionIdName, String sessionIdValue) {
        specification.sessionId(sessionIdName,sessionIdValue);
    }

    public void filter(Filter filter) {specification.filter(filter);}
    ...
    public RequestSpecification value() {return specification;}
    ...
    public void resetSpecification() {this.requestBody = null;}

    private RequestSpecification specification() { return value();}
    ...
    public void clearRequestBody() {this.requestBody = null;}
